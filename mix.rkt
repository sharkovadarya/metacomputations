...
(define mix
  `((read program division vs0)
    (init (:= pp0 (caadr program))
          (:= pending `((,pp0 ,(init-state-immutable (car vs0) (cadr vs0)))))
          (:= marked '())
    ...
    (label-loop (if (equal? labels '()) label-not-found label-loop-continue))

    (label-not-found (return `(label_not_found)))

    (label-loop-continue (:= pp1 (car labels))
                         (:= labels (cdr labels))
    ...
    (block-loop (if (equal? bb '()) block-loop-stop block-loop-continue))

    (block-loop-continue (:= command (car bb))
                         (:= bb (cdr bb))
    ...
    (do-static-assignment (:= vs (state-set-immutable vs x (eval-exp vs exp)))
                          (goto block-loop))
